...
exports.handler = async function(event){
    ...
    try {
        ...
        const result = await uploadVideoToDropbox(token, file);
...
async function uploadVideoToDropbox(token,file){

    const path = `/WeddingCamera/Videos/${file.filename}`;
            ...
            const finish = await fetch("https://content.dropboxapi.com/2/files/upload_session/finish", {
                ...
                headers:{
                    ...
                    "Dropbox-API-Arg":JSON.stringify({
                        cursor:{session_id:startData.session_id, offset},
                        commit:{
                            path,
                            mode:"add",
                            autorename:true
